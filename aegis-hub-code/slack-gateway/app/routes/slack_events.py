...
def _slack_text_for_query_error(incident_id: str, exc: Exception) -> str:
    if isinstance(exc, QueryProcessorError):
    ...
    if isinstance(exc, httpx.TimeoutException):
        return (
            ...
            "Do not retry immediately — check again in a minute."
        )
    return (
...
async def _handle_mention(
    ...
    if not question.strip():
        slack_web_api.post_message(
            ...
            text=(
                ...
                "(example: `@Aegis {incident_id} what caused this error?`)."
            ),
            thread_ts=thread_ts,
        )
        return
    ...
    try:
        ...
        if _is_session_not_ready(exc):
            logger.warning(
                "QP session never ready for %s after %s attempts",
                incident_id,
                SESSION_RETRY_ATTEMPTS,
            )
            return
