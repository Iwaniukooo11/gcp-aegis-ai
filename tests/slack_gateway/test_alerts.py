...
class TestReceiveAlert:
    def test_posts_formatted_message_to_default_channel(self, client, sg_slack_web_api):
        payload = {**_ALERT_BASE, "formatted_message": "*🚨 INC-2026-000042* — java-api OOM", "fallback_text": "fallback"}
        with patch.object(sg_slack_web_api, "post_message", return_value={"ok": True, "ts": "1.1"}) as mock_post:
    ...
    def test_fallback_text_used_when_formatted_message_empty(self, client, sg_slack_web_api):
        payload = {**_ALERT_BASE, "formatted_message": "", "fallback_text": "Fallback: OOM detected in java-api"}
        with patch.object(sg_slack_web_api, "post_message", return_value={"ok": True, "ts": "2.2"}) as mock_post:
            resp = client.post("/v1/internal/incidents/alert", json=payload)

        assert resp.status_code == 200
        assert mock_post.call_args.kwargs["text"] == "Fallback: OOM detected in java-api"

    def test_both_messages_empty_returns_400(self, client, sg_slack_web_api):
    ...
    def test_slack_api_failure_returns_500(self, client, sg_slack_web_api):
        payload = {**_ALERT_BASE, "formatted_message": "alert text", "fallback_text": ""}
        with patch.object(sg_slack_web_api, "post_message", side_effect=RuntimeError("Slack API down")):
            resp = client.post("/v1/internal/incidents/alert", json=payload)
