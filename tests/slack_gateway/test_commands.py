...
class TestLatestIncidentsCommand:
    def test_acks_immediately_with_ephemeral_message(self, client, sg_slack_commands, signed_slack_form):
        with patch.object(sg_slack_commands, "_schedule_fetch_and_post") as mock_schedule:
        ...
        mock_schedule.assert_called_once_with(10, "http://test-response-url", "C_TEST", "U_ENGINEER")

    def test_happy_path_formats_and_posts_incident_list(
        self, sg_slack_commands, sg_qp_client, sg_slack_web_api, sample_bq_rows
    ):
        qp_result = {
        ...
        with patch.object(sg_qp_client, "get_latest_incidents", return_value=qp_result) as mock_qp, \
            ...
            asyncio.run(sg_slack_commands._fetch_and_post(10, "http://test-response-url", "C_TEST", "U_ENGINEER"))
    ...
    def test_custom_limit_passed_to_scheduler(self, client, sg_slack_commands, signed_slack_form):
        with patch.object(sg_slack_commands, "_schedule_fetch_and_post") as mock_schedule:
            _post_cmd(client, signed_slack_form, text="5")

        mock_schedule.assert_called_once_with(5, "http://test-response-url", "C_TEST", "U_ENGINEER")

    def test_non_numeric_limit_text_defaults_to_10(self, client, sg_slack_commands, signed_slack_form):
        with patch.object(sg_slack_commands, "_schedule_fetch_and_post") as mock_schedule:
            _post_cmd(client, signed_slack_form, text="not-a-number")

        mock_schedule.assert_called_once_with(10, "http://test-response-url", "C_TEST", "U_ENGINEER")

    def test_empty_list_posts_no_incidents_message(self, sg_slack_commands, sg_qp_client, sg_slack_web_api):
    ...
    def test_qp_failure_posts_error_via_response_url(self, sg_slack_commands, sg_qp_client, sg_slack_web_api):
