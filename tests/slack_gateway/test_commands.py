...
def _post_cmd(client, text: str = "", response_url: str = "http://test-response-url") -> object:
    return client.post(
        "/slack/commands",
        data={
...
class TestLatestIncidentsCommand:
    ...
    def test_custom_limit_passed_to_qp(self, client, sg_qp_client, sg_slack_web_api):
        with patch.object(sg_qp_client, "get_latest_incidents", return_value={"incidents": [], "count": 0}) as mock_qp, \
             patch.object(sg_slack_web_api, "post_to_response_url"):
            _post_cmd(client, text="5")

        mock_qp.assert_called_once_with(limit=5)

    def test_non_numeric_limit_text_defaults_to_10(self, client, sg_qp_client, sg_slack_web_api):
        with patch.object(sg_qp_client, "get_latest_incidents", return_value={"incidents": [], "count": 0}) as mock_qp, \
             patch.object(sg_slack_web_api, "post_to_response_url"):
            _post_cmd(client, text="not-a-number")

        mock_qp.assert_called_once_with(limit=10)

    def test_empty_list_posts_no_incidents_message(self, client, sg_qp_client, sg_slack_web_api):
        with patch.object(sg_qp_client, "get_latest_incidents", return_value={"incidents": [], "count": 0}), \
             patch.object(sg_slack_web_api, "post_to_response_url") as mock_post_url:
            _post_cmd(client)
    ...
    def test_qp_failure_posts_error_via_response_url(self, client, sg_qp_client, sg_slack_web_api):
    ...
    def test_unknown_command_returns_ephemeral_error(self, client):
        resp = client.post(
            "/slack/commands",
            data={"command": "/unknown-cmd", "text": "", "response_url": "http://x"},
        )
        assert resp.status_code == 200
