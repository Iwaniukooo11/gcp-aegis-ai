...
class TestUrlVerification:
    def test_challenge_echoed_back(self, client):
...
class TestAppMention:
    def test_happy_path_calls_qp_and_posts_to_slack(self, client, sg_qp_client, sg_slack_web_api):
        qp_result = {
        ...
        with patch.object(sg_qp_client, "query_incident", return_value=qp_result) as mock_qp, \
            ...
            resp = client.post(
                "/slack/events",
                json=_make_mention_event("<@UBOT> INC-2026-000042 why is memory spiking"),
            )
    ...
    def test_no_incident_id_posts_error_hint(self, client, sg_qp_client, sg_slack_web_api):
        with patch.object(sg_qp_client, "query_incident") as mock_qp, \
            ...
            resp = client.post(
                "/slack/events",
                json=_make_mention_event("<@UBOT> what is the current status"),
            )
    ...
    def test_qp_failure_posts_fallback_error_to_slack(self, client, sg_qp_client, sg_slack_web_api):
        with patch.object(sg_qp_client, "query_incident", side_effect=RuntimeError("QP unreachable")), \
             patch.object(sg_slack_web_api, "post_message", return_value={"ok": True}) as mock_post:
            resp = client.post(
                "/slack/events",
                json=_make_mention_event("<@UBOT> INC-2026-000042 help"),
            )
    ...
    def test_thread_ts_forwarded_to_slack(self, client, sg_qp_client, sg_slack_web_api):
        with patch.object(sg_qp_client, "query_incident", return_value={"slack_text": "All clear."}), \
             patch.object(sg_slack_web_api, "post_message", return_value={"ok": True}) as mock_post:
            resp = client.post(
                "/slack/events",
                json=_make_mention_event("<@UBOT> INC-2026-000042 status", thread_ts="555.555"),
            )

        assert resp.status_code == 200
        assert mock_post.call_args.kwargs.get("thread_ts") == "555.555"

    def test_non_app_mention_event_ignored(self, client, sg_qp_client, sg_slack_web_api):
        with patch.object(sg_qp_client, "query_incident") as mock_qp, \
             patch.object(sg_slack_web_api, "post_message") as mock_post:
            resp = client.post(
                "/slack/events",
                json={"type": "event_callback", "event": {"type": "message", "text": "hello there"}},
            )
    ...
    def test_unknown_top_level_event_type_ignored(self, client):
        resp = client.post("/slack/events", json={"type": "app_rate_limited"})
        assert resp.status_code == 200
        assert resp.json() == {"ok": True}
